...
A sound-reactive 3D garden. Play an instrument, sing, clap, or tap your desk to grow wireframe flowers and trees, with glitter, drifting coloured lights, and dotted trails. C# / ASP.NET Core processes the audio; Three.js renders the cream 3D scene.

## Run

Install the .NET 10 SDK, open a terminal in this folder, and run:

```powershell
dotnet run --project MelodyGarden.csproj
```

Open http://localhost:5196 in Chrome or Edge. On Windows you can also double-click START.cmd. Stop the existing garden server first if that port is already occupied.

Click Listen, allow your microphone, stay quiet for half a second, then tap, clap, sing, or play an instrument. Stop listening before changing the input under Controls. A USB audio interface is supported as a normal audio input; MIDI is not implemented. Camera is optional. Drag to orbit, scroll to zoom, or Reset view. Auto-rotate can be turned off in Controls.
